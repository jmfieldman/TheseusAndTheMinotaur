...
static const char* s_voxel_frag_src =
    ...
    " *   u_wall_stone_c: grain_intensity, grain_scale, 0, 0 */\n"
    ...
    "\n"
    "/* Fitted wall slab pattern. N = round(seg_len / avg_width) slabs\n"
    ...
    "float wall_stone(float local_pos, float seg_len, vec2 world_uv) {\n"
    ...
    "    float slab_w = seg_len / n;\n"
    "\n"
    "    /* Which slab are we in? */\n"
    "    float col = clamp(floor(local_pos / slab_w), 0.0, n - 1.0);\n"
    "    float local_x = clamp((local_pos - col * slab_w) / slab_w, 0.0, 1.0);\n"
    "\n"
    "    /* Slab identity: include world-space segment start for uniqueness */\n"
    "    float seg_start = world_uv.x - local_pos;\n"
    "    vec2 sid = vec2(col, seg_start * 17.3);\n"
    "\n"
    "    /* Distance from slab edges (0 at edge, 1 at center) */\n"
    "    float edge_dist = min(local_x, 1.0 - local_x) * 2.0;\n"
    "\n"
    "    /* Mortar: thin dark line at slab boundaries */\n"
    "    float mortar = smoothstep(0.0, m_width, edge_dist);\n"
    "    float mortar_shade = mix(1.0 - m_dark, 1.0, mortar);\n"
    "\n"
    "    /* Bevel: wider gradient simulating rounded slab edges */\n"
    "    float bevel = smoothstep(0.0, b_width, edge_dist);\n"
    "    float bevel_shade = mix(1.0 - b_dark, 1.0, bevel);\n"
    "\n"
    ...
    "    vec2 grain_off = vec2(hash2d(sid), hash2d(sid + vec2(73.0, 157.0))) * 100.0;\n"
    "    float grain = fbm((world_uv + grain_off) * g_scale);\n"
    "    float grain_mod = mix(1.0 - g_int, 1.0 + g_int, grain);\n"
    "\n"
    "    return mortar_shade * bevel_shade * color_var * grain_mod;\n"
    "}\n"
    "\n"
    "/* Directional light */\n"
    ...
    "            }\n"
    "            base_color *= wall_stone(local_pos, seg_len, world_uv);\n"
    "        }\n"
    "    }\n"
    "\n"
