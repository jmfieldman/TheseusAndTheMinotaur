...
static const char* s_voxel_frag_src =
    ...
    "            vec3 abs_n = abs(N);\n"
    "            if (abs_n.y > 0.5) {\n"
    "                /* Top face: diamond plate pattern (use local pos so\n"
    "                 * the pattern rotates with the platform mesh) */\n"
    "                vec2 dp = fract(v_local_pos.xz * 10.0);\n"
    ...
    "                float grain = sin(v_local_pos.x * 80.0) * sin(v_local_pos.z * 80.0) * 0.015;\n"
