...
| `death_anim.h / death_anim.c` | Death animation system. Decomposes an actor's body mesh into a 4×4×4 grid of rigid voxel particles that scatter with physics (gravity, floor-height awareness via `tile_physics`, pit fall-through). Each `DeathVoxel` tracks position, velocity, rotation, angular velocity, scale, and color. Renders particles using a shared unit-cube VBO with per-particle model matrix and color override via `glVertexAttrib4f`. Supports reverse playback for undo (lerp reversal in 6.9a, keyframe-based in 6.9b). Death types (SQUISH, WALK_INTO, SPIKE, PETRIFY, PIT_FALL) define scatter patterns — generic burst used as placeholder until Steps 6.10–6.14. |
