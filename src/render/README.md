...
| `renderer.h / renderer.c` | Core renderer. Creates shared quad VBO (unit quad 0–1), compiles two built-in shaders (flat color UI + textured UI), and manages the orthographic projection matrix (screen-space pixels, origin top-left). Provides `renderer_begin_frame()` / `renderer_end_frame()`. |
