...
| `biome_config.h / biome_config.c` | Biome configuration loader. Defines `BiomeConfig` struct hierarchy (palette, wall style, decorations, lanterns, prefabs) and loads from JSON via cJSON. `biome_config_defaults()` provides stone-labyrinth fallback values. `biome_config_load()` reads a biome JSON file and fills the config. Prefabs are inline-defined voxel clusters (up to 24 boxes each). |
