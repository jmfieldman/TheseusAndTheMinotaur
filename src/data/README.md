...
| `settings.h / settings.c` | Global settings (not per-save-slot). `Settings` struct: music_volume, sfx_volume, fullscreen, resolution. Loads/saves YAML via libyaml. `settings_default()` provides sensible defaults. Stored at `{save_dir}/settings.yml`. |
